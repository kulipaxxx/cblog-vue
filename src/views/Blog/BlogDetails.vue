<template>
  <div>
    ...
    <div class="mblog">
      ...
      <ArticleComment :blogId="blog.id"></ArticleComment>
    </div>
...
export default {
  ...
  methods: {
    click() {
      console.log("是否登錄：" + this.hasLogin);
      if (this.hasLogin) {
        this.hasLike = !this.hasLike;
        console.log("haslike:" + this.hasLike)
        let status;
        if (this.hasLike) status = 1;
        else status = 0;
        console.log("點讚狀態為：" + status);
  ...
  created() {
    ...
    if (this.$store.getters.getUser != null) {
      getRelation(id, this.$store.getters.getUser.id).then((res) => {
        //查询当前用户是否点赞
        console.log(id);
        ...
        if (flag) {
          this.hasLike = true;
        } else this.hasLike = false;
      });
    }
    likeCount(id).then((res) => {
      //查询点赞数量
      console.log(res);
      if (res.data.data.count != null) this.count = res.data.data.count;
